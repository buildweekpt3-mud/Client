...
We used arrays, objects, classes and 2d arrays for room generation.

## Justification

For the 2d arrays we made the room generation generate rooms by rows and connect them after each row while making them.

## Runtime

Our room generation has a runtime of O(n) because it goes through every room the map only once
